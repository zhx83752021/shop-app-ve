...
| **生产环境** | 线上正式运行 | https://your-shop.vercel.app  |
...
### 1. 文件上传与 Vercel Blob
Vercel Serverless 环境下，本地文件系统（`public/uploads`）是**只读**的。
- **当前状态**：项目默认使用本地存储，仅适用于开发环境。
- **上线方案**：在生产环境中，必须配置 `Vercel Blob` 或阿里云 `OSS`。
- **临时建议**：在后台管理上传图片时，建议直接输入图片的外链 URL（如 Pexels 或 Unsplash 链接），避免使用本地上传功能。

### 2. 环境变量配置
在 Vercel 控制台中配置以下关键环境变量：
- `DATABASE_URL`: 你的线上数据库链接（Supabase / Vercel Postgres）。
- `JWT_SECRET`: 一个复杂的随机字符串。
- `CORS_ORIGIN`: 设置为你的前端域名（例如 `https://your-shop.vercel.app`）。

### 3. 构建流程
项目根目录下已提供 `build-all.js`，Vercel 会自动调用它来一键构建三个端。
请确保在部署前，`backend/dist` 文件夹已被正确包含或在构建脚本中生成。
